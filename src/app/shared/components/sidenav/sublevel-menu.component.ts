    ...
    <ul *ngIf="collapsed && data.items && data.items.length > 0"
      ...
      <li *ngFor="let item of data.items" class="sublevel-nav-item">
        <a class="sublevel-nav-link" *ngIf="item.items && item.items.length > 0" (click)="handleClick(item)">
          <i class="sublevel-link-icon material-icons">radio_button_unchecked</i>
          <span class="sublevel-link-text" @fadeInOut *ngIf="collapsed">{{item.label}}</span>
          <i class="menu-collapse-icon" *ngIf="item.items && collapsed" [ngClass]="!item.expanded ? 'fal fa-angle-right' : 'fal fa-angle-down'">
          </i>
        </a>
        <a href="" class="sublevel-nav-link" *ngIf="!item.items || (item.items && item.items.length === 0)"
